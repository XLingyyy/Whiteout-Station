"""Generate conservative v0.8 performances directly on each current NPC skeleton.

The previous pipeline retargeted full-body motion from a differently proportioned
game rig. These clips author only a small, audited set of major bones on the exact
VRM skeleton used by each map actor. Face, eye, hair, finger, and cloth bones keep
their imported reference pose.
"""
...
CHARACTERS = (
    {
        ...
        "safe_idle": "/Game/WindStation/Art/AnimeNPC/GuHeng/Animations/AN_GuHeng_Idle",
    ...
    },
    {
        ...
        "safe_idle": "/Game/WindStation/Art/AnimeNPC/YeChengV10/Animations/AN_YeCheng_V10_Idle",
...
RELAXED = {
    UPPER_CHEST: (6.0, 0.0, 0.0),
    HEAD: (8.0, 0.0, 0.0),
    L_SHOULDER: (0.0, 0.0, 0.0),
    R_SHOULDER: (0.0, 0.0, 0.0),
    L_UPPER_ARM: (0.0, 0.0, 0.0),
...
def idle(t: float) -> dict[str, tuple[float, float, float]]:
    breath = wave(t)
    sway = wave(t, phase=0.25)
    return pose(
        **{
            SPINE: (0.45 * breath, 0.0, 0.35 * sway),
            CHEST: (-0.30 * breath, 0.0, -0.25 * sway),
            UPPER_CHEST: (6.0 + 0.35 * breath, 0.0, 0.20 * sway),
            HEAD: (8.0 - 0.30 * breath, 0.0, -0.35 * sway),
            L_UPPER_ARM: (0.25 * sway, 0.45 * breath, 0.0),
            R_UPPER_ARM: (-0.25 * sway, -0.45 * breath, 0.0),
        }
    )
...
def walk(t: float) -> dict[str, tuple[float, float, float]]:
    stride = wave(t)
    lift_left = max(0.0, wave(t))
    lift_right = max(0.0, -wave(t))
    return pose(
        **{
            HIPS: (0.0, 0.0, 1.8 * wave(t, 2.0)),
            SPINE: (0.0, 0.0, -1.8 * stride),
            CHEST: (0.0, 0.0, 1.3 * stride),
            L_UPPER_ARM: (-7.0 * stride, 0.0, 0.0),
            R_UPPER_ARM: (7.0 * stride, 0.0, 0.0),
            L_UPPER_LEG: (13.0 * stride, 0.0, 0.0),
            R_UPPER_LEG: (-13.0 * stride, 0.0, 0.0),
            L_LOWER_LEG: (-15.0 * lift_left, 0.0, 0.0),
            R_LOWER_LEG: (-15.0 * lift_right, 0.0, 0.0),
            L_FOOT: (7.0 * lift_left, 0.0, 0.0),
            R_FOOT: (7.0 * lift_right, 0.0, 0.0),
        }
    )
...
def acknowledge(t: float) -> dict[str, tuple[float, float, float]]:
    ...
    return pose(
        **{
            HEAD: (8.0 + 7.0 * nod, 0.0, 0.0),
            NECK: (2.5 * nod, 0.0, 0.0),
            UPPER_CHEST: (6.0 + 1.3 * nod, 0.0, 0.0),
        }
    )


def consider(t: float) -> dict[str, tuple[float, float, float]]:
    emphasis = pulse(t, 0.52, 0.38)
    return pose(
        **{
            HEAD: (8.0 + 2.0 * emphasis, 0.0, 7.0 * emphasis),
            NECK: (0.0, 0.0, 2.5 * emphasis),
            UPPER_CHEST: (6.0, 0.0, -1.7 * emphasis),
            R_SHOULDER: (0.0, -2.0 * emphasis, 0.0),
            R_UPPER_ARM: (0.0, 4.0 * emphasis, 0.0),
        }
    )


def reassure(t: float) -> dict[str, tuple[float, float, float]]:
    open_amount = pulse(t, 0.50, 0.42)
    return pose(
        **{
            HEAD: (8.0 - 2.0 * open_amount, 0.0, 0.0),
            UPPER_CHEST: (6.0 - 1.5 * open_amount, 0.0, 0.0),
            L_SHOULDER: (0.0, -4.0 * open_amount, 0.0),
            R_SHOULDER: (0.0, 4.0 * open_amount, 0.0),
            L_UPPER_ARM: (-5.0 * open_amount, -4.0 * open_amount, 0.0),
            R_UPPER_ARM: (5.0 * open_amount, 4.0 * open_amount, 0.0),
        }
    )
...
def reject(t: float) -> dict[str, tuple[float, float, float]]:
    ...
    return pose(
        **{
            HEAD: (8.0, 0.0, 7.5 * shake),
            NECK: (0.0, 0.0, 2.3 * shake),
            L_UPPER_ARM: (0.0, -3.0 * brace, 0.0),
            R_UPPER_ARM: (0.0, 3.0 * brace, 0.0),
        }
    )
...
def alarmed(t: float) -> dict[str, tuple[float, float, float]]:
    ...
    return pose(
        **{
            HEAD: (8.0 - 5.0 * amount, 0.0, 0.0),
            NECK: (-2.5 * amount, 0.0, 0.0),
            UPPER_CHEST: (6.0 - 3.0 * amount, 0.0, 0.0),
            L_SHOULDER: (0.0, 4.0 * amount, 0.0),
            R_SHOULDER: (0.0, -4.0 * amount, 0.0),
            L_UPPER_ARM: (-5.0 * amount, -6.0 * amount, 0.0),
            R_UPPER_ARM: (5.0 * amount, 6.0 * amount, 0.0),
        }
    )
...
def author_animation(
    ...
    try:
        ...
        for bone_name in bone_names:
            ...
            safe_idle_transform = unreal.AnimationLibrary.get_bone_pose_for_time(
                safe_idle,
                bone_name,
                0.0,
                False,
    ...
    unreal.log(
        f"WS_V08_ANIMATION_GENERATED character={character['name']} "
...
def main() -> None:
    for character in CHARACTERS:
        ...
        safe_idle = require_asset(character["safe_idle"])
        if safe_idle.get_editor_property("skeleton") != skeleton:
            raise RuntimeError(
                f"Safe idle uses the wrong skeleton for {character['name']}"
            )
    ...
    unreal.log(
        f"WS_V08_ANIMATION_GENERATION_COMPLETE assets="
