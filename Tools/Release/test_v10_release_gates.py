...
try:
    ...
    from .validate_release_v10 import validate_release_artifact
    ...
    from .run_v10_shipping_smoke import EXPECTED_ROUTES, SCENARIOS
except ImportError:
    from v10_gate_common import (
        AGENT_RUNTIME_REL,
        DISTRIBUTION_CLASS,
...
ALLOWED_ADDITION = f"{DIFF_ONLY_ROOT}/GuHeng/AnimationsV10"
...
def make_repository(tmp_path: Path) -> tuple[Path, str, str]:
    ...
    write_text(
        repo,
        "docs/PROTECTED_CHARACTER_ASSETS_v1.0.json",
        json.dumps(
            {
                "version": "1.0.0",
    ...
    git(repo, "add", "--", "docs/PROTECTED_CHARACTER_ASSETS_v1.0.json")
...
def make_valid_artifact(
    ...
    artifact_root = (
        tmp_path / "artifacts" / f"WhiteoutStation-v1.0-Win64-{run_id}"
    )
    ...
    input_summary = {
        "schema": "whiteout.v1.0.real-input-smoke.v1",
        "passed": True,
        "scenarios": [
            {
                "passed": True,
...
def test_stale_v04_false_green_artifact_is_rejected(tmp_path: Path) -> None:
    ...
    assert not report.passed
    assert any("v1.0" in error or "v0.4" in error for error in report.errors)
...
def test_stale_v10_build_timestamp_is_rejected(tmp_path: Path) -> None:
    repo, commit, tree = make_repository(tmp_path)
    artifact_root, manifest_path, validation_time = make_valid_artifact(
        tmp_path,
        repo,
...
def test_missing_protected_manifest_fails_closed(tmp_path: Path) -> None:
    repo, _commit, _tree = make_repository(tmp_path)
    manifest = repo / "docs/PROTECTED_CHARACTER_ASSETS_v1.0.json"
    manifest.unlink()
    report = validate_protected_assets(repo)
    assert not report.passed
...
def test_untracked_protection_manifest_warns_until_final_mode(
    ...
    repo, _commit, _tree = make_repository(tmp_path)
    relative_path = "docs/PROTECTED_CHARACTER_ASSETS_v1.0.json"
