"""Fail-closed source gate for the Whiteout Station v0.9 release candidate.
...
V09_REQUIRED_ASSETS = (
    *(
        ...
        f"AnimationsV09/{prefix}_{suffix}.uasset"
        for root, prefix in (
            ("GuHeng", "AN_GuHeng"),
    ...
    "docs/baseline_v0.9/UI_character_gu_v09_face_1600x1000.png",
    "docs/baseline_v0.9/UI_character_ye_v09_face_1600x1000.png",
    "docs/baseline_v0.9/UI_lookat_near_1600x1000.png",
    "docs/baseline_v0.9/UI_lookat_side_1600x1000.png",
    "docs/baseline_v0.9/UI_lookat_far_1600x1000.png",
)
...
def validate_versions(repo_root: Path) -> GateReport:
    ...
    if not rules_path.is_file():
        report.error(f"Missing v0.9 rules file: {RULES_REL}")
    else:
        try:
    ...
    if not agent_path.is_file():
        report.error(f"Missing v0.9 Agent runtime file: {AGENT_RUNTIME_REL}")
    else:
        try:
    ...
    legacy_scan_files = [
        ...
        *(
            path
            for path in repo_root.joinpath("Tools/Release").glob("*v09*.py")
            if not path.name.startswith("test_")
        ),
        *(
            path
            for path in repo_root.joinpath("Tools/Release").glob("v09_*.py")
            if not path.name.startswith("test_")
        ),
    ]
    for path in sorted(set(legacy_scan_files)):
        ...
        for line_number, line in enumerate(lines, 1):
            ...
            if any(marker in lowered for marker in LEGACY_LOAD_MARKERS):
                report.error(
                    f"Current runtime/v0.9 tool loads a legacy runtime file at "
...
def validate_v09_feature_contract(repo_root: Path) -> GateReport:
    report = GateReport()
    try:
    ...
    if not isinstance(characters, dict):
        report.error("v0.9 initial_state.characters must be an object")
    else:
        expected_characters = {"player", "gu_heng", "ye_cheng"}
        if set(characters) != expected_characters:
            report.error(
                "v0.9 initial character set mismatch: "
                f"{sorted(characters)!r}"
            )
        for character_id, state in characters.items():
            if not isinstance(state, dict):
                report.error(
                    f"v0.9 character state must be an object: {character_id}"
                )
                continue
            for metric in CHARACTER_METRICS:
                ...
                if (
                    ...
                    report.error(
                        f"v0.9 {character_id}.{metric} must be within 0..10"
                    )
    ...
    if not isinstance(thresholds, dict) or not thresholds:
        report.error("v0.9 balance.thresholds must be a non-empty object")
    else:
        for threshold, value in thresholds.items():
            if (
                ...
                report.error(
                    f"v0.9 threshold {threshold} must be within 0..10"
                )

    try:
        tracked = tracked_paths(repo_root)
    except GateError as exc:
        report.error(f"Cannot list v0.9 feature assets: {exc}")
        tracked = set()
    for relative_path in V09_REQUIRED_ASSETS:
        path = repo_root / relative_path
        if relative_path not in tracked or not path.is_file():
            report.error(f"Missing tracked v0.9 feature asset: {relative_path}")
    ...
    for relative_path, markers in source_contracts.items():
        try:
            ...
            report.error(f"Cannot read v0.9 source contract {relative_path}: {exc}")
            continue
        for marker in markers:
            if marker not in text:
                report.error(
                    f"v0.9 source contract marker missing from "
                    f"{relative_path}: {marker}"
                )

    if not report.errors:
        report.detail(
            "v0.9 feature contract passed: 0..10 state scale, HUD/cursor, "
...
def validate_protected_assets(
    ...
    *,
    ...
    if str(manifest.get("version", "")).lower().lstrip("v") not in {
        ...
        report.error("Protected asset manifest version must identify v0.9")
...
def validate_source(
    ...
    *,
    ...
    report.merge(validate_v09_feature_contract(repo_root))
...
def print_report(report: GateReport) -> None:
    ...
    for error in report.errors:
        print(f"ERROR: {error}")
    print(
        "SOURCE GATE v0.9: PASS"
        if report.passed
        else f"SOURCE GATE v0.9: FAIL ({len(report.errors)} error(s))"
    )
