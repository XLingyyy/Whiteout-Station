"""Validate one explicitly selected, fresh Whiteout Station v0.5 artifact.
...
{
  "schema": "whiteout.v0.5.release-manifest.v1",
  "version": "0.5.0",
  "run_id": "YYYYMMDDTHHMMSSZ-<commit8>-<nonce>",
  "artifact_root_name": "WhiteoutStation-v0.5-Win64-<run_id>",
...
try:
    ...
    from .validate_source_v05 import (
        AGENT_BASENAME,
        OLD_AGENT_BASENAME,
        OLD_RULES_BASENAME,
        RULES_BASENAME,
...
MINIMUM_PACKAGE_SIZES = {
    ...
    "Windows/WhiteoutStation/Content/Rules/WhiteoutStationRules.v0.5.json": 100,
    "Windows/WhiteoutStation/Content/Agents/AgentRuntime.v0.5.json": 100,
    "README_v0.5.txt": 10,
...
def _validate_manifest_identity(
    ...
    if not isinstance(run_id, str) or not RUN_ID_PATTERN.fullmatch(run_id):
        ...
        return None
    expected_root_name = f"WhiteoutStation-v0.5-Win64-{run_id}"
    if artifact_root.name != expected_root_name:
        report.error(
            f"Artifact directory must be the unique v0.5 run directory "
...
def _validate_package_versions(
    ...
    rules_rel = (
        ...
        "WhiteoutStationRules.v0.5.json"
    )
    agent_rel = (
        "Windows/WhiteoutStation/Content/Agents/"
        "AgentRuntime.v0.5.json"
    )
    try:
    ...
    try:
        first_line = (
            artifact_root.joinpath("README_v0.5.txt")
            .read_text(encoding="utf-8-sig")
            .splitlines()[0]
        )
        if "v0.5" not in first_line or "v0.4" in first_line:
            report.error("Packaged README first line must identify v0.5 only")
    ...
    *,
    ...
    for relative_path in REQUIRED_PACKAGE_FILES:
        ...
        if relative_path not in actual_paths or not path.is_file():
            report.error(f"Missing required v0.5 artifact file: {relative_path}")
            continue
    ...
    for relative_path in actual_paths:
        ...
        if "v0.4" in lowered:
            report.error(
                f"Historical v0.4 file is forbidden in v0.5 artifact: {relative_path}"
            )
        if lowered.endswith(
            (
            ...
            report.error(
                f"Legacy runtime config is forbidden in v0.5 artifact: {relative_path}"
            )
...
def validate_release_artifact(
    ...
    *,
    ...
    if not artifact_root.is_dir():
        ...
        return report
    if not artifact_root.name.startswith("WhiteoutStation-v0.5-Win64-"):
        report.error(
            "Artifact root must be an explicitly selected unique v0.5 directory; "
    ...
    if not report.errors:
        report.detail(
            f"Validated fresh v0.5 artifact {artifact_root.name} "
...
def print_report(report: GateReport) -> None:
    ...
    for error in report.errors:
        print(f"ERROR: {error}")
    print(
        "RELEASE VALIDATION v0.5: PASS"
        if report.passed
        else f"RELEASE VALIDATION v0.5: FAIL ({len(report.errors)} error(s))"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Exact unique v0.5 artifact directory; no directory auto-discovery occurs",
    )
