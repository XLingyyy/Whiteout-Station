"""Drive the packaged v0.6 Demo through real keyboard and mouse input."""
...
def load_event_log(
    ...
    if (
        event_log.get("rules_version") != "0.6.0"
...
def validate_artifact_root(artifact_root: Path) -> tuple[Path, Path]:
    ...
    if not root.name.startswith(ARTIFACT_PREFIX):
        raise SmokeError("Artifact root is not a unique v0.6 archive")
    run_id = root.name[len(ARTIFACT_PREFIX) :]
    if not RUN_ID_PATTERN.fullmatch(run_id):
        raise SmokeError("Artifact root has an invalid v0.6 run id")
...
def run_input_smoke(artifact_root: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(
        ...
        report = {
            "schema": "whiteout.v0.6.real-input-smoke.v1",
            "passed": True,
...
def main() -> int:
    ...
    try:
        ...
        print(f"REAL INPUT SMOKE v0.6: FAIL: {exc}")
        return 1
    print(f"REAL INPUT SMOKE v0.6: PASS (2/2) summary={summary_path}")
    return 0
