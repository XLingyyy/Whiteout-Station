"""Validate one explicitly selected, fresh Whiteout Station v0.9 artifact.
...
{
  "schema": "whiteout.v0.9.release-manifest.v1",
  "version": "0.9.0",
  "distribution_class": "local_review_only",
  "run_id": "YYYYMMDDTHHMMSSZ-<commit8>-<nonce>",
  "artifact_root_name": "WhiteoutStation-v0.9-Win64-<run_id>",
...
try:
    ...
    from .validate_source_v09 import (
        AGENT_BASENAME,
...
MINIMUM_PACKAGE_SIZES = {
    ...
    "Windows/WhiteoutStation/Content/Rules/WhiteoutStationRules.v0.9.json": 100,
    "Windows/WhiteoutStation/Content/Agents/AgentRuntime.v0.9.json": 100,
    "README_v0.9.txt": 10,
...
def _validate_manifest_identity(
    ...
    if not isinstance(run_id, str) or not RUN_ID_PATTERN.fullmatch(run_id):
        ...
        return None
    expected_root_name = f"WhiteoutStation-v0.9-Win64-{run_id}"
    if artifact_root.name != expected_root_name:
        report.error(
            f"Artifact directory must be the unique v0.9 run directory "
...
def _validate_package_versions(
    ...
    rules_rel = (
        ...
        "WhiteoutStationRules.v0.9.json"
    )
    agent_rel = (
        "Windows/WhiteoutStation/Content/Agents/"
        "AgentRuntime.v0.9.json"
    )
    try:
    ...
    try:
        ...
        report.error(str(exc))
    try:
        readme_text = artifact_root.joinpath("README_v0.9.txt").read_text(
            encoding="utf-8-sig"
        )
        first_line = readme_text.splitlines()[0]
        if "v0.9" not in first_line or "v0.4" in first_line:
            report.error("Packaged README first line must identify v0.9 only")
    ...
    *,
    ...
    for relative_path in REQUIRED_PACKAGE_FILES:
        ...
        if relative_path not in actual_paths or not path.is_file():
            report.error(f"Missing required v0.9 artifact file: {relative_path}")
            continue
    ...
    for relative_path in actual_paths:
        ...
        if "v0.4" in lowered:
            report.error(
                f"Historical v0.4 file is forbidden in v0.9 artifact: {relative_path}"
            )
        if lowered.endswith(
            (
            ...
            report.error(
                f"Legacy runtime config is forbidden in v0.9 artifact: {relative_path}"
            )
...
def _validate_smoke_evidence(
    ...
    if isinstance(input_summary, dict):
        if (
            ...
            != "whiteout.v0.9.real-input-smoke.v1"
            or input_summary.get("passed") is not True
        ):
            report.error("v0.9 real-input smoke summary is not a passing report")
        scenarios = input_summary.get("scenarios")
        if not isinstance(scenarios, list) or len(scenarios) != 2:
            report.error("v0.9 real-input smoke must contain two scenarios")
        else:
            ...
            for scenario in scenarios:
                if (
                    ...
                    report.error("v0.9 real-input scenario did not pass")
                    continue
            ...
            if len(cursor_checks) < 6:
                report.error(
                    "v0.9 real-input smoke lacks six panel cursor-center checks"
                )
            for check in cursor_checks:
                if (
                    ...
                    report.error("v0.9 cursor-center check did not pass")
                    continue
                ...
                if (
                    ...
                    report.error("v0.9 cursor-center evidence is malformed")
            if not any(name.endswith("_story_10") for name in capture_names):
                report.error(
                    "v0.9 real-input smoke did not capture the tenth opening line"
                )
    elif input_summary is not None:
        report.error("v0.9 real-input smoke summary root must be an object")
    ...
    if isinstance(shipping_summary, dict):
        if (
            ...
            != "whiteout.v0.9.shipping-smoke.v1"
            or shipping_summary.get("passed") is not True
        ):
            report.error("v0.9 Shipping smoke summary is not a passing report")
        ...
        if (
            ...
            report.error("v0.9 Shipping smoke route matrix is incomplete")
        ...
        if (
            ...
            report.error("v0.9 dialogue-history probe is incomplete")
        performances = shipping_summary.get("performance_probes")
        if not isinstance(performances, list) or len(performances) != 2:
            report.error("v0.9 Shipping smoke must contain two NPC performances")
        else:
            actions = {
            ...
            if actions != {"talk_gu_heng", "talk_ye_cheng"}:
                report.error(
                    "v0.9 Shipping smoke lacks both NPC movement/reaction probes"
                )
    elif shipping_summary is not None:
        report.error("v0.9 Shipping smoke summary root must be an object")
...
def validate_release_artifact(
    ...
    *,
    ...
    if not artifact_root.is_dir():
        ...
        return report
    if not artifact_root.name.startswith("WhiteoutStation-v0.9-Win64-"):
        report.error(
            "Artifact root must be an explicitly selected unique v0.9 directory; "
    ...
    if not report.errors:
        report.detail(
            f"Validated fresh v0.9 artifact {artifact_root.name} "
...
def print_report(report: GateReport) -> None:
    ...
    for error in report.errors:
        print(f"ERROR: {error}")
    print(
        "RELEASE VALIDATION v0.9: PASS"
        if report.passed
        else f"RELEASE VALIDATION v0.9: FAIL ({len(report.errors)} error(s))"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Exact unique v0.9 artifact directory; no directory auto-discovery occurs",
    )
