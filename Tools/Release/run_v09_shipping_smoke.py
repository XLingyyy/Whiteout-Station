"""Run the v0.9 Shipping routes and AI fail-safe smoke scenarios.
...
def validate_artifact_root(artifact_root: Path) -> tuple[Path, Path]:
    ...
    if not root.name.startswith(ARTIFACT_PREFIX):
        raise SmokeError(f"Artifact root is not a unique v0.9 archive: {root.name}")
    run_id = root.name[len(ARTIFACT_PREFIX) :]
    if not RUN_ID_PATTERN.fullmatch(run_id):
        raise SmokeError(f"Artifact root has an invalid v0.9 run id: {run_id}")
    ...
    if agent.get("runtime_version") != "0.9.0":
        raise SmokeError("Packaged Agent runtime_version must be 0.9.0")
...
def validate_event_log(
    ...
    if event_log.get("rules_version") != "0.9.0":
...
def run_performance_probe(
    ...
    *,
    ...
    expected_fields = {
        "schema": "whiteout.v0.9.performance-probe.v1",
        "action_id": action_id,
...
def run_shipping_smoke(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        ...
        evidence_root = staging_root / "Evidence"
        report = {
            "schema": "whiteout.v0.9.shipping-smoke.v1",
            "passed": True,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Exact unique WhiteoutStation-v0.9-Win64-<run_id> artifact root",
    )
    ...
    try:
        ...
        print(f"SHIPPING SMOKE v0.9: FAIL: {exc}")
        return 1
    print(f"SHIPPING SMOKE v0.9: PASS (12/12) summary={summary_path}")
    return 0
