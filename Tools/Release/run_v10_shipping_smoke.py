"""Run the v1.0 Shipping routes and AI fail-safe smoke scenarios.
...
try:
    from .v10_gate_common import MANIFEST_REL, RUN_ID_PATTERN
except ImportError:
    from v10_gate_common import MANIFEST_REL, RUN_ID_PATTERN
...
ARTIFACT_PREFIX = "WhiteoutStation-v1.0-Win64-"
...
SCENARIOS = (
    ...
    Scenario("missing_key_cost", "cost", "default_missing_key", 0),
...
def validate_artifact_root(artifact_root: Path) -> tuple[Path, Path]:
    ...
    if not root.name.startswith(ARTIFACT_PREFIX):
        raise SmokeError(f"Artifact root is not a unique v1.0 archive: {root.name}")
    run_id = root.name[len(ARTIFACT_PREFIX) :]
    if not RUN_ID_PATTERN.fullmatch(run_id):
        raise SmokeError(f"Artifact root has an invalid v1.0 run id: {run_id}")
    ...
    if agent.get("runtime_version") != "1.0.0":
        raise SmokeError("Packaged Agent runtime_version must be 1.0.0")
    ...
    if agent.get("llm_enabled") is not True:
        raise SmokeError("Packaged Agent runtime must default AI integration on")
    ...
    if (
        ...
        or performance.get("movement_intents") != EXPECTED_MOVEMENT_INTENTS
        or performance.get("reaction_actions") != EXPECTED_REACTION_ACTIONS
    ):
        raise SmokeError("Packaged Agent performance contract mismatch")
...
def validate_event_log(
    ...
    if event_log.get("rules_version") != "1.0.0":
...
def run_performance_probe(
    ...
    *,
    ...
    performance_path = (
        runtime_root / "Saved/Logs/WhiteoutStation_PerformanceProbe.json"
    )
    ...
    for field, expected in expected_fields.items():
        if performance.get(field) != expected:
            raise SmokeError(
                f"{scenario_id}: performance field {field} mismatch"
            )
    try:
...
def run_shipping_smoke(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        ...
        evidence_root = staging_root / "Evidence"
        report = {
            "schema": "whiteout.v1.0.shipping-smoke.v1",
            "passed": True,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Exact unique WhiteoutStation-v1.0-Win64-<run_id> artifact root",
    )
    ...
    try:
        ...
        print(f"SHIPPING SMOKE v1.0: FAIL: {exc}")
        return 1
    print(f"SHIPPING SMOKE v1.0: PASS (12/12) summary={summary_path}")
    return 0
