"""Drive the packaged v1.0 Demo through real keyboard and mouse input."""
...
def load_event_log(
    ...
    if (
        event_log.get("rules_version") != "1.0.0"
...
def validate_artifact_root(artifact_root: Path) -> tuple[Path, Path]:
    ...
    if not root.name.startswith(ARTIFACT_PREFIX):
        raise SmokeError("Artifact root is not a unique v1.0 archive")
    run_id = root.name[len(ARTIFACT_PREFIX) :]
    if not RUN_ID_PATTERN.fullmatch(run_id):
        raise SmokeError("Artifact root has an invalid v1.0 run id")
...
def main() -> int:
    ...
    try:
        ...
        print(f"REAL INPUT SMOKE v1.0: FAIL: {exc}")
        return 1
    print(f"REAL INPUT SMOKE v1.0: PASS (2/2) summary={summary_path}")
    return 0
