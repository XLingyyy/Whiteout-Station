"""Fail-closed source gate for the Whiteout Station v0.6 release candidate.
...
def validate_versions(repo_root: Path) -> GateReport:
    ...
    if not rules_path.is_file():
        report.error(f"Missing v0.6 rules file: {RULES_REL}")
    else:
        try:
    ...
    if not agent_path.is_file():
        report.error(f"Missing v0.6 Agent runtime file: {AGENT_RUNTIME_REL}")
    else:
        try:
    ...
    legacy_scan_files = [
        ...
        *(
            path
            for path in repo_root.joinpath("Tools/Release").glob("*v06*.py")
            if not path.name.startswith("test_")
        ),
        *(
            path
            for path in repo_root.joinpath("Tools/Release").glob("v06_*.py")
            if not path.name.startswith("test_")
        ),
    ]
    for path in sorted(set(legacy_scan_files)):
        ...
        for line_number, line in enumerate(lines, 1):
            ...
            if any(marker in lowered for marker in LEGACY_LOAD_MARKERS):
                report.error(
                    f"Current runtime/v0.6 tool loads a legacy runtime file at "
...
def validate_protected_assets(
    ...
    *,
    ...
    if not isinstance(manifest, dict):
        ...
        return report
    if str(manifest.get("version", "")).lower().lstrip("v") not in {"0.6", "0.6.0"}:
        report.error("Protected asset manifest version must identify v0.6")
...
def print_report(report: GateReport) -> None:
    ...
    for error in report.errors:
        print(f"ERROR: {error}")
    print(
        "SOURCE GATE v0.6: PASS"
        if report.passed
        else f"SOURCE GATE v0.6: FAIL ({len(report.errors)} error(s))"
    )
