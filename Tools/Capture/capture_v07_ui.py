#!/usr/bin/env python3
"""Capture v0.7 HUD, crosshair, interaction prompt, and dialogue baselines."""
...
def run_capture(width: int, height: int, timeout_seconds: float) -> None:
    ...
    if any(psutil.pid_exists(pid) for pid in active):
        raise TimeoutError(f"Capture timed out for {width}x{height}: {sorted(active)}")
...
def main() -> int:
    ...
    print("v0.7 UI baseline capture complete", flush=True)
    return 0
