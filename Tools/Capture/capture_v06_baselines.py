#!/usr/bin/env python3
"""Capture the v0.6 onboarding, dialogue, menu, and ending baselines."""
...
def run_capture(
    ...
    if not active:
        raise RuntimeError(
            f"Unreal Editor did not start for {mode} {width}x{height}"
        )

    while (
        time.monotonic() < deadline
        and any(psutil.pid_exists(pid) for pid in active)
    ):
        time.sleep(1.0)
    if any(psutil.pid_exists(pid) for pid in active):
        raise TimeoutError(
            f"Capture timed out for {mode} {width}x{height}: {sorted(active)}"
        )
...
def main() -> int:
    ...
    print("v0.6 baseline capture complete", flush=True)
    return 0
