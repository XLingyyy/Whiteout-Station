...
The Whiteout Station v1.1 competition artifact is classified as `onsite_competition_demo`: the project author runs it directly on the same device at the venue, without uploading or transferring the executable or embedded asset data to the organizer, judges or audience. If a later submission requires uploading, transferring or publicly providing the build, obtain written permission from Noanoa or replace the hair with an asset carrying suitable terms. The source `.vrm` remains outside the repository.
