# Whiteout Station v1.0 - 构建、运行与验收

日期：2026-07-29

v1.0 直接维护在 `main`，不使用额外工作树。用户地图、模型、骨骼和材质
保持不变；当前动画只位于两个 `AnimationsV10` 目录。
...
python -X utf8 -m pytest Tools/Release/test_v10_release_gates.py -q
python -X utf8 -m unittest Tools.Release.test_scan_secrets -v
python -X utf8 Tools/Release/scan_secrets.py --history
python -X utf8 Tools/Release/validate_source_v10.py --repo-root . --final
```
...
& 'G:\UnrealEngine\UE_5.8\Engine\Binaries\Win64\UnrealEditor-Cmd.exe' `
  'G:\Whiteout Station\WhiteoutStation\WhiteoutStation.uproject' `
  -run=pythonscript `
  -script='G:\Whiteout Station\Tools\Editor\audit_v10_npc_animations.py' `
...
运行配置为
`WhiteoutStation/Content/Agents/AgentRuntime.v1.0.json`。只在当前启动
...
```powershell
python -X utf8 Tools/Release/run_v10_input_smoke.py --artifact-root <artifact>
python -X utf8 Tools/Release/run_v10_shipping_smoke.py --artifact-root <artifact>
python -X utf8 Tools/Release/create_release_manifest_v10.py `
  --repo-root . --artifact-root <artifact> --run-id <run-id> `
  --source-ref <commit> --build-timestamp-utc <timestamp>
python -X utf8 Tools/Release/validate_release_v10.py `
