...
默认关闭 AI，靠作者选项完成对话。对话最多三轮，首轮成功提交 1 AP，后续两轮不重复收费；第三轮留下的提议可确认或取消收尾，不开放第四轮自由聊天。在线模式在设置页填写 provider、BaseURL、model 和仅保留于本进程的 Key 后启用。在线按 A 意图解析、局部规则规划、B 合并表达顺序处理，A 最多 3 秒／1600 tokens，B 最多 7 秒，每子意图 640 tokens、整条最多 3840 tokens，最多 6 个子意图、两次请求，整轮最多 10 秒，无自动重试。DeepSeek 显式关闭默认 thinking，避免解析预算全部被推理占用。
