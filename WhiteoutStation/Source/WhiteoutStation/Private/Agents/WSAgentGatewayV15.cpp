	...
	TFunction<void(bool, const FWSCanonicalIntent&, const FString&)> Completion)
{
	...
	const FString Instruction = TEXT(
		...
		"An instruction that the listener must obey the player's arrangements or stop delaying is a clear interpersonal command: command,relationship,unknown query,empty target_action_id,current speaker as target_character. It affects cooperation only; no physical action is executed. Do not require a named physical task for this interpersonal act. "
		"A promise to heat the repair room next phase uses promise,commitment,consequence,repair_generator,gu_heng,promise_condition=heat_repair_room. New offers use commitment=proposed. Explicit agreement to that pending promise uses the SAME speech_act=promise and subject fields, commitment=confirm_pending, needs_clarification=false. Rejection uses reject_pending. This is not a command to heat now. Pending context is separate from committed turns; resolved_from_turn must not exceed latest_context_turn, even when a pending promise exists. "
