...
static UIKRigDefinition* GenerateMannequinIK(UVrmAssetListObject* vrmAssetList, FString path_forSK, FString Footer) {
...

	{
		...
		const FReferenceSkeleton& SourceRefSkeleton = VRMGetRefSkeleton(sk);
		const FString SpineEnd = SourceRefSkeleton.FindRawBoneIndex(TEXT("spine_05")) != INDEX_NONE
			? TEXT("spine_05")
			: TEXT("spine_03");
		const FString NeckEnd = SourceRefSkeleton.FindRawBoneIndex(TEXT("neck_02")) != INDEX_NONE
			? TEXT("neck_02")
			: TEXT("neck_01");

		TArray<TT> table = {
			{TEXT("Spine"),		TEXT("spine_01"),				SpineEnd,},
			{TEXT("Neck"),		TEXT("neck_01"),				NeckEnd,},
		...
		for (auto& t : table) {
			if (SourceRefSkeleton.FindRawBoneIndex(*t.s1) == INDEX_NONE
				|| SourceRefSkeleton.FindRawBoneIndex(*t.s2) == INDEX_NONE) {
				continue;
			}
...
bool VRMConverter::ConvertIKRig(UVrmAssetListObject *vrmAssetList) {
	...
	if (vrmAssetList && VRMConverter::Options::Get().IsGenerateRigIK()) {
...
#else
		{
			...
			rtgDataTable[1].BaseSKName = TEXT("/Game/WindStation/Art/Characters/Engineer/SK_WS_Engineer.SK_WS_Engineer");
